...
> **Domain authority:** `BUSINESS_RULES.md`  
> **Schema authority:** `DATABASE.md`
...
Transactional behavior is normative in `BUSINESS_RULES.md`. Returns:
...
Behavior is the transaction defined in `BUSINESS_RULES.md`. The target job must be visible as well as published. Returns credit ID, job ID, boost ID, start, and end timestamps.
...
- Keep API contracts synchronized with `DATABASE.md`, `BUSINESS_RULES.md`, and UI flows in the same change.
